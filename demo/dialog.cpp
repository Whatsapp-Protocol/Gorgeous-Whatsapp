...
void Dialog::CreateCommonDbTable() {
...

	{
		...
		QSqlQuery query(common_db_);
		query.exec(
			"CREATE TABLE IF NOT EXISTS users (cc text, phone text,primary "
			"key(cc,phone));");
	}
}
...
void Dialog::LoadFromDb() {
	...
	db_.setDatabaseName(QApplication::applicationDirPath() + "\\" +
		ui->ccs->currentText() + ui->phones->currentText());
	if (!db_.open()) {
		QMessageBox::warning(0, QString::fromStdWString(L"数据库错误"),
			db_.lastError().text());
		return;
...
void Dialog::on_contactlists_customContextMenuRequested(const QPoint& pos) {
	...
	//删除菜单
	QAction* delete_contact = new QAction(QString::fromStdWString(L"delete"), this);
